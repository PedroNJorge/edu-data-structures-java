public class ArrayListIntSet implements IntSet {
	...
	public boolean equals(IntSet s) {
		...
		for (int i = 0; i < this.size(); i++) {
			if (!s.contains(this.elem[i])) return false;
		}
		return true;
	}

	public IntSet intersection(IntSet s) {
		int minSize = ((s.size() > this.size()) ? this.size() : s.size());
		IntSet result = new ArrayListIntSet(minSize);
		for (int i = 0; i < this.size(); i++) {
			if (s.contains(this.elem[i])) result.add(this.elem[i]);
		}
		return result;
	}
}
